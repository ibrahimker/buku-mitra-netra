...
function filter_sdm_post_type_content($content) {
    ...
    if ($post->post_type == "sdm_downloads") {//Handle the content for sdm_downloads type post
        ...
        $isset_download_thumbnail = isset($item_download_thumbnail) && !empty($item_download_thumbnail) ? '<img class="sdm_post_thumbnail_image" alt="Cover Buku" src="' . $item_download_thumbnail . '" />' : '';
        ...
        if(is_user_logged_in()){
            ...
            $content.='<div class="sdm_post_download_section"><div class="sdm_download_link alert alert-danger">' .'<p>Anda harus login terlebih dahulu untuk dapat mengunduh buku ini</p>' . '</div></div>';
        }

        if (!empty($isset_item_file_size)) {//Show file size info
            $content .= '<div class="sdm_post_download_file_size">';
            $content .= '<span class="sdm_post_download_size_label">' . __('Ukuran File: ', 'simple-download-monitor') . '</span>';
            $content .= '<span class="sdm_post_download_size_value">' . $isset_item_file_size . '</span>';
            $content .= '</div>';
        }

        if (!empty($isset_item_isbn)) {//Show ISBN info
            $content .= '<div class="sdm_post_download_version">';
            $content .= '<span class="sdm_post_download_version_label">' . __('ISBN: ', 'simple-download-monitor') . '</span>';
            $content .= '<span class="sdm_post_download_version_value">' . $isset_item_isbn . '</span>';
            $content .= '</div>';
        }

        if (!empty($isset_item_pengarang)) {//Show Pengarang info
            $content .= '<div class="sdm_post_download_version">';
            $content .= '<span class="sdm_post_download_version_label">' . __('Pengarang: ', 'simple-download-monitor') . '</span>';
            $content .= '<span class="sdm_post_download_version_value">' . $isset_item_pengarang . '</span>';
            $content .= '</div>';
        }

        if (!empty($isset_item_penerbit)) {//Show Pengarang info
            $content .= '<div class="sdm_post_download_version">';
            $content .= '<span class="sdm_post_download_version_label">' . __('Penerbit: ', 'simple-download-monitor') . '</span>';
            $content .= '<span class="sdm_post_download_version_value">' . $isset_item_penerbit . '</span>';
            $content .= '</div>';
        }
