<?php
/**
* Plugin Name: Buku Mitra Netra
* Plugin URI: https://www.tipsandtricks-hq.com/simple-wordpress-download-monitor-plugin
* Description: Plugin to manage book which can be downloaded in your website. Created by editing simple download manager plugin
* Version: 3.3.2
* Author: Ibrahim Nurandita Isbandiputra
* Author URI: https://www.linkedin.com/in/ibrahimisbandiputra
* License: GPL2
*/
if (!defined('ABSPATH')) {
    exit;
...
class simpleDownloadManager {
    ...
    public function display_sdm_description_meta_box($post) {  // Description metabox
        _e('Berikan deskripsi buku anda.', 'simple-download-monitor');
        echo '<br /><br />';
    ...
    public function display_sdm_other_details_meta_box($post){ //Other details metabox
        ...
        $isbn = get_post_meta($post->ID, 'sdm_item_isbn', true);
        $isbn = isset($isbn) ? $isbn : '';

        $pengarang = get_post_meta($post->ID, 'sdm_item_pengarang', true);
        $pengarang = isset($pengarang) ? $pengarang : '';

        $penerbit = get_post_meta($post->ID, 'sdm_item_penerbit', true);
        $penerbit = isset($penerbit) ? $penerbit : '';
        
        echo '<div class="sdm-download-edit-filesize">';
        _e('Ukuran File: ', 'simple-download-monitor');
        echo '<br />';
        ...
        _e('ISBN: ', 'simple-download-monitor');
        echo '<br />';
        echo ' <input type="text" name="sdm_item_isbn" value="' . $isbn . '" size="20" />';
        echo '<p class="description">'.__('Enter the ISBN number for this book (example value: 081299860X).', 'simple-download-monitor').'</p>';
        echo '</div>';

        echo '<div class="sdm-download-edit-version">';
        _e('Pengarang: ', 'simple-download-monitor');
        echo '<br />';
        echo ' <input type="text" name="sdm_item_pengarang" value="' . $pengarang . '" size="20" />';
        echo '<p class="description">'.__('Masukkan nama pengarang buku (Misal: Ibrahim Nurandita).', 'simple-download-monitor').'</p>';
        echo '</div>';

        echo '<div class="sdm-download-edit-version">';
        _e('Penerbit: ', 'simple-download-monitor');
        echo '<br />';
        echo ' <input type="text" name="sdm_item_penerbit" value="' . $penerbit . '" size="20" />';
        echo '<p class="description">'.__('Masukkan nama penerbit buku (Misal: Mitra Netra).', 'simple-download-monitor').'</p>';
        echo '</div>';
    ...
    public function sdm_save_other_details_meta_data($post_id) {  // Save Statistics Upload metabox
        ...
        if (isset($_POST['sdm_item_isbn'])){
            update_post_meta($post_id, 'sdm_item_isbn', $_POST['sdm_item_isbn']);
        }

        if (isset($_POST['sdm_item_pengarang'])){
            update_post_meta($post_id, 'sdm_item_pengarang', $_POST['sdm_item_pengarang']);
        }

        if (isset($_POST['sdm_item_penerbit'])){
            update_post_meta($post_id, 'sdm_item_penerbit', $_POST['sdm_item_penerbit']);
        }
